import rawData from "./data/aperturasKnowledgeBase.json";
...
export type CleanAperturaRecord = {
  recordId: string;
...
export type DiscardedAperturaEvent = {
  recordId: string;
  storeCode: string | null;
  storeName: string;
  storeLabel: string;
  ...
  sourceRowNumber: number;
  keptOpeningTime: string;
  keptOpeningDateTime: string;
  secondsFromKeptOpening: number;
  reason: string;
};
...
export type AperturasKnowledgeBase = {
  meta: KnowledgeBaseMeta;
  records: CleanAperturaRecord[];
  discardedEvents: DiscardedAperturaEvent[];
  stores: KnowledgeBaseStore[];
  summaryByDate: SummaryByDate[];
};

export type AperturasQuery = {
  range?: string | null;
...
const aperturasKnowledgeBase = rawData as AperturasKnowledgeBase;
const DEFAULT_PARSE_YEAR = Number(
  aperturasKnowledgeBase.meta.dateRange.end.slice(0, 4),
);

export const APERTURAS_DATE_RANGE_LABEL = formatRangeLabel(
  aperturasKnowledgeBase.meta.dateRange.start,
  aperturasKnowledgeBase.meta.dateRange.end,
);

export function getAperturasDateRangeLabel() {
  return APERTURAS_DATE_RANGE_LABEL;
}

export function getRangeLabel(query: AperturasQuery) {
  const window = resolveDateWindow(query);
  return formatRangeLabel(window.startDate, window.endDate);
}

export function findAperturas(query: AperturasQuery = {}) {
  const window = resolveDateWindow(query);
  const normalizedStoreQuery = normalizeKey(query.storeQuery ?? "");

  return aperturasKnowledgeBase.records.filter((record) => {
    const inDateWindow =
...
export function getDiscardedEventsForRecords(records: CleanAperturaRecord[]) {
  const allowedRecordIds = new Set(records.map((record) => record.recordId));

  return aperturasKnowledgeBase.discardedEvents.filter((event) =>
    allowedRecordIds.has(event.recordId),
  );
}

export function buildAperturasLookupResult(query: AperturasQuery = {}) {
  const matches = findAperturas(query);
  const limitedMatches = matches.slice(0, normalizeLimit(query.limit));
  const discardedEvents = getDiscardedEventsForRecords(matches);
  ...
  return {
    status: "ok",
    knowledgeBase: {
      dateRange: APERTURAS_DATE_RANGE_LABEL,
      distinctStores: aperturasKnowledgeBase.meta.distinctStores,
      cleanOpenings: aperturasKnowledgeBase.meta.cleanOpenings,
      discardedEvents: aperturasKnowledgeBase.meta.discardedEvents,
      cleaningRule: "Se conserva la primera activacion por sucursal y fecha.",
    },
    appliedFilters: {
      range: getRangeLabel(query),
      storeQuery: query.storeQuery ?? null,
    },
    summary: {
      matchingOpenings: matches.length,
...
function resolveDateWindow(query: AperturasQuery) {
  const fallbackStart = aperturasKnowledgeBase.meta.dateRange.start;
  const fallbackEnd = aperturasKnowledgeBase.meta.dateRange.end;
