...
if (!inputJsonPath || !outputXlsxPath) {
  throw new Error(
    "Usage: node export_aperturas_clean_workbook.mjs <input-json> <output-xlsx>",
  );
}
...
const openingsSheet = workbook.worksheets.add("Aperturas limpias");
...
summarySheet.getRange("A1").value = "Base limpia de aperturas Propasa";
...
summarySheet.getRange("A3:B10").values = [
  ...
  ["Regla de limpieza", "Primera activacion por sucursal y fecha"],
  ["Aperturas limpias", records.length],
  ["Eventos descartados", totalDiscarded],
  ["Sucursales", stores.length],
  ["Dias cubiertos", uniqueDates],
];
summarySheet.getRange("A12:C12").values = [["Fecha", "Aperturas", "Descartados"]];
summarySheet.getRange("A12:C12").style = {
  font: { bold: true, color: "#FFFFFF" },
  fill: { color: "#1E40AF" },
};
summarySheet.getRange(`A13:C${12 + summaryByDate.length}`).values = summaryByDate.map(
  (item) => [item.date, item.cleanOpenings, item.discardedEvents],
);

openingsSheet.getRange("A1:G1").values = [[
  "Fecha",
  "Sucursal",
  "Hora de apertura",
  "Eventos detectados",
...
openingsSheet.getRange("A1:G1").style = {
  font: { bold: true, color: "#FFFFFF" },
  fill: { color: "#1E40AF" },
};
openingsSheet.getRange(`A2:G${records.length + 1}`).values = records.map((record) => [
  record.date,
  record.storeLabel,
  record.openingTime,
...
if (discardedEvents.length > 0) {
  duplicatesSheet.getRange(`A2:G${discardedEvents.length + 1}`).values =
    discardedEvents.map((item) => [
      ...
      item.storeLabel,
      item.time,
      item.keptOpeningTime,
      item.secondsFromKeptOpening,
